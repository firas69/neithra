...
4. Inspect `lib/views/app_shell.dart` for the bottom-navigation structure.
5. Inspect `lib/providers/test_library_provider.dart` and
   `lib/providers/history_provider.dart`.
6. Inspect `lib/providers/session_provider.dart` for pause/resume session state.
7. Inspect `lib/services/database/sqlite_service.dart` for durable local persistence.
8. Inspect `lib/widgets/answer_input_widget.dart` for compositional question rendering.
9. Run:
...
2. Import a JSON exam into the persistent library.
3. Search or open an exam card and start an exam session.
4. Answer questions in a focused exam interface.
